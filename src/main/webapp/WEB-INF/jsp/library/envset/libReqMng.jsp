...
<body>
							...
							<c:choose>
								<c:when test="${fn:length(lib) > 0}">
									<c:forEach var="row" items="${lib}" varStatus="status">
										<tr>
											<td style="text-align:left; padding-left: 10px;">${row.libName} (${row.libManageCode})</td>
											<td>${row.libId}</td>
											<td>
												<input type="checkbox" name="limitYN" class="check" id="limitYN_${row.libId}"
													<c:if test="${row.limitYn eq 'Y' }">checked</c:if> value="Y"/>
												<label for="limitYN_${row.libId}" class="check_wrap"></label>
											</td>
											<td>
												<input type="checkbox" name="listYN" class="check" id="listYN_${row.libId}"
													<c:if test="${row.listYn eq 'Y' }">checked</c:if> value="Y"/>
												<label for="listYN_${row.libId}" class="check_wrap"></label>
											</td>
											<td>
												<input type="text" name="limitDate1" class="input_100" id="reqLimitFrom_${row.libId}" value="${row.limitDate1}">
												~
												<input type="text" name="limitDate2" class="input_100" id="reqLimitTo_${row.libId}" value="${row.limitDate2}">
											</td>
											<td>
	...
	<script type="text/javascript" src="<c:url value="/resources/library/js/envset/libReqMng.js?v=${version }" />"></script>
</body>
</html>